...
outerColumns:
	...
	for p := range g.AllPoints() {
		if g.Get(p) == '#' {
			galaxyPoints = append(galaxyPoints, adjustPoint(p, emptyRows, emptyColumns, 1))
		}
	}
...
func adjustPoint(p common.Point, emptyRows, emptyColums []int, scale int) common.Point {
	...
	return p.Add(common.NewPoint(deltaX*scale, deltaY*scale))
}
...
outerColumns:
	...
	for p := range g.AllPoints() {
		if g.Get(p) == '#' {
			galaxyPoints = append(galaxyPoints, adjustPoint(p, emptyRows, emptyColumns, 1000000-1))
		}
	}
	// Now sum all the lengths
	var total int
